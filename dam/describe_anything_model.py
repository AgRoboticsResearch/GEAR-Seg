...
from dam.model.conversation import SeparatorStyle, conv_templates
from dam.model.mm_utils import KeywordsStoppingCriteria, process_image, tokenizer_image_token
from dam.model import get_model_name_from_path, load_pretrained_model
...
class DescribeAnythingModel(nn.Module):
    ...
    def crop_image(cls, pil_img, mask_np, crop_mode, min_box_w=48, min_box_h=48):
        ...
        if crop_mode == "crop":
            ...
            # crop image and mask 即以目标框的左上角为起点，向外“右下”扩展两倍的宽高范围。
            x0, y0, w, h = cls.mask_to_box(mask_np)
            img_np = np.asarray(pil_img)
            assert img_np.shape[:2] == mask_np.shape, f"image shape mismatches with mask shape: {img_np.shape}, {mask_np.shape}"
            ...
            # crop image and mask 以mask中心 原来mask的3倍大小
            x0, y0, w, h = cls.mask_to_box(mask_np)
            img_np = np.asarray(pil_img)
            assert img_np.shape[:2] == mask_np.shape, f"image shape mismatches with mask shape: {img_np.shape}, {mask_np.shape}"
            img_h, img_w = img_np.shape[:2]
            ...
            cropped_mask_np = mask_np[max(y0-h, 0):min(y0+2*h, img_h), max(x0-w, 0):min(x0+2*w, img_w)]
            cropped_img_np = img_np[max(y0-h, 0):min(y0+2*h, img_h), max(x0-w, 0):min(x0+2*w, img_w)]
            # cropped_mask_np = mask_np[max(y0-0.5*h, 0):min(y0+1.5**h, img_h), max(x0-0.5*w, 0):min(x0+1.5*w, img_w)]
            # cropped_img_np = img_np[max(y0-0.5*h, 0):min(y0+1.5*h, img_h), max(x0-0.5*w, 0):min(x0+1.5*w, img_w)]
            cropped_pil_img = Image.fromarray(cropped_img_np)
